...
class Client {
     ...
     */
    const DEFAULT_GATEWAY_URL = NLB_PAYMENT_URL;
    ...
    protected function sendRequest($xml, $url) {
        ...
        if ($httpResponse->getErrorCode() || $httpResponse->getErrorMessage()) {
            throw new ClientException('1 Request failed: ' . $httpResponse->getErrorCode() . ' ' . $httpResponse->getErrorMessage());
        }
    ...
    protected function sendJsonApiRequest($dataArray, $path) {
        ...
        if ($httpResponse->getErrorCode() || $httpResponse->getErrorMessage()) {
            throw new ClientException('2 Request failed: ' . $httpResponse->getErrorCode() . ' ' . $httpResponse->getErrorMessage());
        }
        if ($httpResponse->getStatusCode() == 504 || $httpResponse->getStatusCode() == 522) {
            throw new TimeoutException('Request timed-out');
        }
    ...
    public function getOptions($identifier, $args = null, $_ = null) {
        ...
        if ($response->getErrorCode() || $response->getErrorMessage()) {
            throw new ClientException('3 Request failed: ' . $response->getErrorCode() . ' ' . $response->getErrorMessage());
        }
